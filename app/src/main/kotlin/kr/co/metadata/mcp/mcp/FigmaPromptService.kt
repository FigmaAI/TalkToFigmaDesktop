...
/**
 * Service for managing Figma-related prompts
 * Provides strategic guidance and best practices for Figma operations
 */
object FigmaPromptService {
    private val logger = KotlinLogging.logger {}

    /**
     * Register all Figma-related prompts
     */
    fun registerPrompts(server: Server) {
        registerDesignStrategyPrompt(server)
        registerReadDesignStrategyPrompt(server)
    ...
    /**
     * Design Strategy Prompt - Best practices for working with Figma designs
     */
    private fun registerDesignStrategyPrompt(server: Server) {
        server.addPrompt(
            ...
            arguments = listOf(
                PromptArgument(
                    name = "context",
                    description = "Optional context for the design strategy",
                    required = false
                )
            )
        ) { request ->
            val context = request.arguments?.get("context") ?: "general"
            
            GetPromptResult(
                description = "Design strategy for $context",
                messages = listOf(
                    PromptMessage(
                        role = Role.assistant,
                        content = TextContent("""When working with Figma designs, follow these best practices:
...
- Login Screen (main frame)
  ...
  - Helper Links (frame)
    ...
    /**
     * Read Design Strategy Prompt - Best practices for reading Figma designs
     */
    private fun registerReadDesignStrategyPrompt(server: Server) {
        server.addPrompt(
            ...
            arguments = listOf(
                PromptArgument(
                    name = "context",
                    description = "Optional context for the read strategy",
                    required = false
                )
            )
        ) { request ->
            ...
            GetPromptResult(
                description = "Read design strategy for $context",
                messages = listOf(
                    PromptMessage(
                        role = Role.assistant,
                        content = TextContent("""When reading Figma designs, follow these best practices:
...
1. Start with selection:
   ...
   - If no selection ask user to select single or multiple nodes""")
    ...
    /**
     * Text Replacement Strategy Prompt - Systematic approach for replacing text in Figma designs
     */
    private fun registerTextReplacementStrategyPrompt(server: Server) {
        server.addPrompt(
            ...
            arguments = listOf(
                PromptArgument(
                    name = "context",
                    description = "Optional context for the text replacement strategy",
                    required = false
                )
            )
        ) { request ->
            ...
            GetPromptResult(
                description = "Text replacement strategy for $context",
                messages = listOf(
                    PromptMessage(
                        role = Role.assistant,
                        content = TextContent("""# Intelligent Text Replacement Strategy
...
Remember that text is never just text—it's a core design element that must work harmoniously with the overall composition. This chunk-based strategy allows you to methodically transform text while maintaining design integrity.""")
    ...
    /**
     * Annotation Conversion Strategy Prompt - Strategy for converting manual annotations to Figma's native annotations
     */
    private fun registerAnnotationConversionStrategyPrompt(server: Server) {
        server.addPrompt(
            ...
            arguments = listOf(
                PromptArgument(
                    name = "context",
                    description = "Optional context for the annotation conversion strategy",
                    required = false
                )
            )
        ) { request ->
            ...
            GetPromptResult(
                description = "Annotation conversion strategy for $context",
                messages = listOf(
                    PromptMessage(
                        role = Role.assistant,
                        content = TextContent("""# Automatic Annotation Conversion
...
This strategy focuses on practical implementation based on real-world usage patterns, emphasizing the importance of handling various UI elements as annotation targets, not just text nodes.""")
    ...
    /**
     * Swap Overrides Instances Prompt - Guide to swap instance overrides between instances
     */
    private fun registerSwapOverridesInstancesPrompt(server: Server) {
        server.addPrompt(
            ...
            arguments = listOf(
                PromptArgument(
                    name = "context",
                    description = "Optional context for the swap overrides strategy",
                    required = false
                )
            )
        ) { request ->
            ...
            GetPromptResult(
                description = "Swap overrides instances strategy for $context",
                messages = listOf(
                    PromptMessage(
                        role = Role.assistant,
                        content = TextContent("""# Swap Component Instance and Override Strategy
...
- Preserve component relationships by using instance overrides rather than direct text manipulation""")
     ...
     */
    private fun registerReactionToConnectorStrategyPrompt(server: Server) {
        server.addPrompt(
            ...
            arguments = listOf(
                PromptArgument(
                    name = "context",
                    description = "Optional context for the reaction to connector strategy",
                    required = false
                )
            )
        ) { request ->
            ...
            GetPromptResult(
                description = "Reaction to connector strategy for $context",
                messages = listOf(
                    PromptMessage(
                        role = Role.assistant,
                        content = TextContent("""# Strategy: Convert Figma Prototype Reactions to Connector Lines
